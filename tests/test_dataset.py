...
@pytest.mark.parametrize('path,num_workers', list(product(videos, [11])))
def test_dataloader_keyframe(path, num_workers):
    vd = TorchVideoDataset(path, format='tensor', keyframes_only=True, image_tx=None)

    def process_batches(batches):
        pts = torch.cat([b['pts'] for b in batches])
        pts, indices = torch.sort(pts)
        tensors = np.array(torch.cat([b['image'] for b in batches]))
        tensors = tensors[indices]
        return pts.numpy(), tensors
    ...
    pts0,tensors0 = process_batches(batches_serial)
    pts1,tensors1 = process_batches(batches_parallel)
    assert pts0.shape == pts1.shape
    assert (pts0 == pts1).all()
    assert (tensors0 == tensors1).all()